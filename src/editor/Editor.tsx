...
import { Sprite, previewAppearance } from "../engine/Sprite";
import {
  decodeGame,
  deleteDataGame,
  duplicateDataGame,
  ...
  return (
    <div className="fixed inset-0 z-[90] bg-zinc-950 text-white flex flex-col">
      ...
      <div className="flex-1 min-h-0 flex">
        ...
        {tab === "sprite" && selDef && (
          <CostumeStudio def={selDef} update={(patch) => updateDef(selDef.id, patch)} />
        )}
        {tab === "sprite" && !selDef && <Empty msg="Select an actor to draw its costumes." />}
        {tab === "events" && (
          <EventsTab
            draft={draft}
            addEvent={addEvent}
...
const SceneTab: React.FC<any> = ({
  ...
  const preview = (a: ActorDef) => {
    const app = previewAppearance(a);
    return app && app.kind === "emoji" ? app.char : null;
  };
              ...
              <span className="text-lg w-6 text-center">
                {preview(a) ?? (
                  <span className="inline-block w-5 h-5 align-middle">
                    <Sprite app={previewAppearance(a)} sizeCqw={18} />
                  </span>
                )}
              </span>
        ...
        >
          ...
          {draft.scene.instances.map((inst: any) => {
            ...
            return (
              ...
              >
                <Sprite app={previewAppearance(def)} sizeCqw={def.height} />
              </div>
            );
          })}
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div>
          ...
          <select value={draft.lengthBars} onChange={(e) => mutate((d) => (d.lengthBars = Number(e.target.value) as 2 | 4))} className={inp}>
            ...
            <option value={4} className="bg-zinc-800">4 bars — long game</option>
          </select>
        </div>
        <div>
            ...
            <div key={g.id} className="rounded-xl border border-white/10 bg-zinc-800/60 p-3 flex flex-col gap-2">
              <div className="flex items-center gap-2">
                <span className="w-7 h-7 rounded-md flex items-center justify-center text-lg" style={{ background: g.palette.screen }}>
                  {g.actors[0] ? <Sprite app={previewAppearance(g.actors[0])} sizeCqw={26} /> : "🎨"}
                </span>
