...
function updateMicrogame(c: Core, dt: number, dtBeats: number) {
  ...
  /* --- Early exit (internal scheduler behaviour, never surfaced to the player).
     A 4-bar game whose outcome is already decided may end at the halfway
     (beat 8) or three-quarter (beat 12) checkpoint, so a game you solved in
     one second doesn't leave you staring at a finished screen for three more.
     The end only ever snaps to a checkpoint whose door-closing beat has not
     yet begun, so the 4/4 flow is never broken.

     In the original this falls out of the beatscript scheduler: `rest N`
     stores N<<8 and is decremented by deltaTime = (tempo<<8)/150 each frame,
     so script time is inherently quantised. It is a property of the clock,
     not a feature — which is why v2 no longer prints a badge about it. --- */
...
function snapshot(c: Core): EngineSnapshot {
  ...
  if (p.kind === "interlude" && c.mg) {
    ...
    // [v2] Instruction is a stinger: it clears a fraction of a beat into play
    // rather than hanging around for two full beats. See Instruction in
    // Screens.tsx for the timing envelope. `t + 1` because the microgame
    // spawns one beat before control (t runs -1 -> 0 while doors open).
    if (t < 0.15) {
      instruction = c.mg.def.instruction;
      instructionAge = t + 1;
    }
